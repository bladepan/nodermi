(function() {
  ...
  var ObjectRegistry = require('./object_registry');
  var ClassRegistry = require('./class_registry');

  var commonModule = require('./common');
  var privatePrefix = commonModule.privatePrefix;
  var excludeMethods = commonModule.excludeMethods;
  ...
  var Decoder = require('./decoding');
  var FailureDetector = require("./failure_detector");

  var logger = debug('nodermi:server');
  var errorLogger = debug('nodermi:error:server');
  ...
  var Server = (function() {
    ...
    */
    ...
    Server.prototype._requestHandler = function(message, response) {
      ...
      if (logger.enabled) {
        logger(this.serverIdentifier + " got request " + commonModule.toJsonString(message));
      }
    ...
    Server.prototype._handleInvokeFunction = function(sessionId, functionId, thisId, args, source, response) {
      ...
      try {
        ...
        if ((args != null) && args.length > 0) {
          var decoder = new Decoder(this, source, sessionId);
    ...
    Server.prototype.retrieveObj = function(options, callback) {
      ...
      return this.transport.send(destination, message, (function(_this) {
        return function(err, returnMessage) {
          ...
          if (logger.enabled) {
            logger("retrieveObj response :" + commonModule.toJsonString(returnMessage));
          }
